...
void ssd1306_write_command(uint8_t command) {
  HAL_I2C_Mem_Write(SSD1306_I2C, SSD1306_I2C_SA_WRITE,
                    SSD1306_CONTROL_BYTE_COMMAND, 1, &command, 1, 1000);
}

void ssd1306_write_data(uint8_t *buffer, uint16_t size) {
  HAL_I2C_Mem_Write(SSD1306_I2C, SSD1306_I2C_SA_WRITE,
                    SSD1306_CONTROL_BYTE_DATA, 1, buffer, size, 1000);
}
...
void entire_display_off() { ssd1306_write_command(ENTIRE_DISPLAY_OFF); }

void entire_display_on() { ssd1306_write_command(ENTIRE_DISPLAY_ON); }

void set_normal_display() { ssd1306_write_command(SET_NORMAL_DISPLAY); }

void set_inverse_display() { ssd1306_write_command(SET_INVERSE_DISPLAY); }

void set_display_on() { ssd1306_write_command(SET_DISPLAY_ON); }

void set_display_off() { ssd1306_write_command(SET_DISPLAY_OFF); }
...
void set_page_start_address_for_page_addressing_mode(
    uint8_t page) // 0xB0(page0) ~ 0xB7(page7)
{
...
void set_segment_remap(uint8_t mapping) { ssd1306_write_command(mapping); }
...
void set_com_pins_hardware_config(uint8_t com_pin_config,
                                  uint8_t com_left_right_remap) {
...
void set_display_clock_divide_ratio_and_osc_freq(uint8_t divide_ratio,
                                                 uint8_t osc_freq) {
...
uint16_t ssd1306_write_char_cn(SSD1306_CN_FONT font, uint16_t ch) {
  ...
  for (int i = 0; i < font.width; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      if (b & (0x01 << j)) {
        ssd1306_white_pixel(cursor.x + i, cursor.y + j + 8);
      } else {
        ssd1306_black_pixel(cursor.x + i, cursor.y + j + 8);
      }
    }
  }
